import { GoTrueClientOptions, Session } from '@shaple/shaple';
import { DEFAULT_COOKIE_OPTIONS, parseShapleCookie, stringifyShapleSession } from './utils';
...
export abstract class CookieAuthStorageAdapter implements StorageAdapter {
	...
	getItem(key: string): string | Promise<string | null> | null {
		...
		if (value) {
			return JSON.stringify(parseShapleCookie(value));
		}
		...
		return chunks !== null ? JSON.stringify(parseShapleCookie(chunks)) : null;
	}
	...
	setItem(key: string, value: string): void | Promise<void> {
		...
		const sessionStr = stringifyShapleSession(session);
